...
class SwitchAdapter(private val context: Context, private val items: List<String>) :
    ...
    private val states = BooleanArray(items.size) // Стан перемикачів
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(position: Int) {
            ...
            textView.setTextColor(if (states[position]) context.getColor(android.R.color.holo_green_dark) else context.getColor(android.R.color.holo_red_dark))

            switchToggle.isChecked = states[position]
            switchToggle.setOnCheckedChangeListener { _, isChecked ->
                states[position] = isChecked
                textView.text = if (isChecked) "ON" else "OFF"
                textView.setTextColor(if (isChecked) context.getColor(android.R.color.holo_green_dark) else context.getColor(android.R.color.holo_red_dark))
            }
        }
    }
